...
async def findCard(channel_id, search_key):
	if channel_id in MTG_CHANNELS:
	...
	try:
		...
		if len(cards) == 0:
			if set_id:
				...
				return Message(message="Couldn't find a card for '{0}'".format(search_key), cleanup_original=False, cleanup_self=False)
		else:
...
async def mtgFindCards(channel_id, search_keys):
	if channel_id in MTG_CHANNELS:
		response_messages = []
		for search_key in search_keys:
			response_messages.append(mtgFindCard(search_key))
